...
int chooseNext(){
    ...
    for (int i = 0; i < N; i++)
    {
        originalCost+=(arr[i]-closest[i])*(arr[i]-closest[i]);
    }
    for (int k = 0; k < 3; k++)
    {    
        ...
        {
            int tmpCost=0;
            memcpy(tmpClosest, closest, N);
            for (int j = 0; j < N; j++)
            {
                if(abs(arr[j]-closest[j])>abs(arr[j]-i)){
                    tmpClosest[j]=i;
                }
            }
            for (int j = 0; j < N; j++)
            {
                tmpCost+=(arr[j]-tmpClosest[j])*(arr[j]-tmpClosest[j]);
            }
            if(bestCost>tmpCost){
                bestCost=tmpCost;
            }
