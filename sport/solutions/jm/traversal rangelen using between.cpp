...
tree* ConstructTree(ii rangestart, int rangelen, tree* parent) {
	...
	// 시작:
	// pre: [rangestart.first, ... , rangestart.first + rangelen]
	// in:  [rangestart.second, ..., rootindex - 1, rootindex, rootindex + 1, ... , rangestart.second + rangelen]
...
void solve() {
	...
	for (int c = 0; c < C; c++)
	{
		...
		tree* tt = ConstructTree({ 0, 0 }, len - 1, NULL);
