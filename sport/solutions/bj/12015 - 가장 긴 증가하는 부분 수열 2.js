...
function solve() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const newval = arr[i];
    // it(newval);
    if(newval > minlast[minlast.length-1]){
      // minlast[minlast.length-1] = newval;
      minlast.push(newval);
    } else {
      var is = 0;
      var ie = minlast.length;
      // it('minlast ' + minlast, i);
      // var kk = 0;
      var mid;
      while(is < ie){
        mid = Math.floor((is + ie) / 2);
        if(minlast[mid] < newval){
          is = mid + 1;
        } else {
          ie = mid;
        }
        ...
        // it('curcur' + mid);
        // it(minlast);
        // it(minlast[mid], minlast[mid + 1]);
        // if(minlast[mid + 1] == undefined){
        //   mid--;
        //   ie--;
        // }
        // if(minlast[mid] <= newval && minlast[mid+1] > newval){
        //   break;
        // } else if(minlast[mid] > newval) {
        //   //왼쪽으로 가야함.
        //   ie = mid - 1;
        //   mid = Math.floor((is + ie) / 2);
        // } else {
