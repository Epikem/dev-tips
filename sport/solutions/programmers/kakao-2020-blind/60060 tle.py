...
# w,q,r = ["frodo", "front", "frost", "frozen", "frame", "kakao"]	["fro??", "????o", "fr???", "fro???", "pro?"]	[3, 2, 4, 1, 0]
...
def solve(words, queries):
    ...
    for word in words:
        ...
        for i in range(llen):
            ...
            cur=rv+cur
            rev_cur+=v
            suffix_tree[cur.rjust(llen, '?')] += 1
            prefix_tree[rev_cur.ljust(llen, '?')] += 1
    
    # print('all', all)
    # print('suffix_tree', suffix_tree)
    # print('prefix_tree', prefix_tree)
        
    ans = []
    # iter queries
    for q in queries:
        # check if ? appears at start or end
        startq = q[0] == '?'
        endq = q[-1] == '?'
        allq = startq and endq
        # remove all ? from q
        # key = q.replace('?', '')
        key = q
        # print('key=', key, 'startq=', startq, 'endq=', endq)
        if(allq):
            ans.append(all)
        elif(startq):
            ans.append(suffix_tree[key])
        else:
            ans.append(prefix_tree[key])
