...
def solve(records):
    ...
    commands = {'Enter': 0, 'Leave': 1, 'Change':2}
    def handleRecord(record):
        ...
        if command != 2:
            recs.append((command, user))
        # print(command, user, name)
        
    # set the name of user
    def setName(user, name):
        uid_to_name[user] = name
    ...
    for i,v in enumerate(recs):
        command, user = v
        ans.append(f'{uid_to_name[user]}님이 들어왔습니다.' if command == 0 
            else f'{uid_to_name[user]}님이 나갔습니다.')
    
    # handleRecord:  
    # enter: uid에 대한 입장 기록
    # leave: uid에 대한 퇴장 기록
    # change: uid에 대한 이름 변경

    # display:
    # 각 enter, leave 기록에 대한 배열 계산
...
record, result = ["Enter uid1234 Muzi"
    , "Enter uid4567 Prodo"
    , "Leave uid1234"
    , "Enter uid1234 Prodo"
