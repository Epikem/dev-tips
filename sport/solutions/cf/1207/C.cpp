...
void solve()
{
    ...
    for(int t=0;t<T;t++){
        ...
        for(int i=0;i<n;i++){
            ...
            else if(c=='0' and before==1){
                oneland.push_back(curlen);
                // oneland.append(curlen)
                before=0;
                curlen=1;
            }
            else if(c=='1' and before==0){
                zeroland.push_back(curlen);
                curlen=1;
                before=1;
            }
            else if(c=='1' and before==1){
                curlen+=1;
            }
        }
        // zeroland.append(curlen)
        zeroland.push_back(curlen);
        curlen=0;
        ll cost=0;
        if(zeroland.size()==1){
        ...
        for(int i=0;i<zeroland.size();i++){
            ll z=zeroland[i];
            // cout << z << ' ' << i << endl;
            if(i==0 or i==zeroland.size()-1){
                cost+=z*b+(z-1)*a+2*a;
                continue;
            }
            if(z==1){
            ...
            ll costz1=(z-1)*b+(z+2)*a;
            ll costz2=(z-1)*2*b+z*a;
            cost+=min<ll>(costz1,costz2);
        }
        for(int i=0;i<oneland.size();i++){
            ll o=oneland[i];
            cost+=(o+1)*2*b+o*a;
        }
        cout << cost << endl;
