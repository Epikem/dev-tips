...
void solve(){
	...
	for (int tt = 0; tt < t; tt++)
	{
		...
		// 어쨌든 합이 n이므로 임의의 수 a에 대해 b=n-a가 되고,
		// 이진 탐색 냄새가 나는데,
		// 어떻게 구간을 줄어야 하지?
		// 조건 중 하나는 적용됬고, 남은건 결국 a*b = a(n-a) = n 조건 인데,
		// a에 대해 미분하면 -a^2+na -> -2a+n이다. 즉 
		// 
		// -a^2+na-n = 0응?
		...
		ld a = (n + sqrt(n*n - 4 * n)) / 2;
		ld b = (n - sqrt(n*n - 4 * n)) / 2;

		cout << "Y ";
		cout << std::setprecision(9) << a;
		cout << ' ';
		cout << std::setprecision(9) << b;
		cout << '\n';
	}
}
