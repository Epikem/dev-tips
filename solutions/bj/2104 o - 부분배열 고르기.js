...
function getMaxArea(inputs){
  ...
  for(var i = 1; i <= N+1;i++){
    ...
    while(stk.length > 0){
      if(stk[stk.length-1][0] > arr[i]){
        var item = stk.pop();
        h = item[0];
        var prev = (stk.length == 0? 0 : stk[stk.length-1][1]);
        maxarea = Math.max(maxarea, h * (accsum[i-1]-accsum[prev]));
        it(`pop  + ${item}, h:${h}, item1:${item[1]}, prev:${prev}, size: ${h * (accsum[i-1]-accsum[prev])}`);
      } else break;
    } 
    stk.push([arr[i],i]);
    it('push ' + [arr[i], i]);
