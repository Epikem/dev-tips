...
string fliptree(int pos) {

	char terminal = tree[pos];
	if (terminal == 'x') {
		// process subtree
		int nextpos = pos + 1;
		string t1 = fliptree(nextpos);
		nextpos = nextpos + t1.size();
		string t2 = fliptree(nextpos);
		nextpos = nextpos + t2.size();
		string t3 = fliptree(nextpos);
		nextpos = nextpos + t3.size();
		string t4 = fliptree(nextpos);
		
		string result = 'x' + t3 + t4 + t1 + t2;
		return result;
	}
	else { 
		string ss = { terminal }; 
