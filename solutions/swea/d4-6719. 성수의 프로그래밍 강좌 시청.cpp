...
int N, T, K;
// 정렬하여 가장 높은 K개 강좌를 골라, 낮은 거부터 들으면 된다고 생각한다.
// 만약 그보다 낮은 강좌를 대신 고르면 당연히 평균이 낮아지고,
// 순서를 바꾼다면 또한 높은거 다음 낮은걸 다음 강의의 비중이 50%이므로 낮아지게 된다.
// 따라서 정렬하려 가장 높은 K개 강좌를 낮은 거부터 들으면 된다.
...
void solve()
{
    ...
    for (int t = 0; t < T; t++)
    {
        ...
        {
            // cout << lec[i] << ' ';
            ans = (ans + (double)lec[i]) / 2.0;
        }

        cout << '#' << t + 1 << ' ' << fixed << setprecision(8) << ans << '\n';
    }
}
