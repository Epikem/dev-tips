...
ll calcCost(int start){
    ...
    if(arr[start]!=arr[start+1]) {
        ret=calcCost(start+1);
        return ret;
    } else {
        if(start>0){
            if(arr[start-1]==arr[start]+1){
                ret=min(ret, calcCost(start+1)+costs[start]+costs[start-1]);
            } 
        }
        ret=min(ret, calcCost(start+1)+2*costs[start]);
    }
    return ret;
}

ll getCost(int cur, int p){ // start를 p번 올린 경우의 최소 비용
